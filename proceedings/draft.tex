...
\end{equation}
where we have assumed periodic boundary conditions. In the continuum limit, the topological susceptibility diverges owing solely to the $x^*=0$ term \cite{bietenholz2018}, a divergence that also exists in QCD \cite{bruno2014}.
...
\end{equation}
This final relation will allow us to probe the behavior of the topological susceptibility in the nontopological NLSM by considering the topological charge in the nontrivial theory. \chris{Might want to rephrase this? Lots of ``nons'' here.}
...
\begin{figure}[h!]
    \centering
      ...
      \caption{\label{fig:bergluscher} Comparison with \cite{berg1981}. First panel: internal energy compared with analytic  energy (Eq.~\ref{eq:analytic_energy}). Second panel: magnetic susceptibility compared with literature values. \chris{Can you please remake these plots so that they both have x-axis labels? Please also make the labels on both axes bigger.}}
\end{figure}
...
\begin{figure}[h!]
    \centering
      ...
      \caption{\label{fig:bietenholz} Dimensionless magnetic susceptibility, $\chi_t L^2$, \chris{In the text/y-axis label you say this is $\chi_t\xi_2^2$?} as a  function of flow time $\tau/t_0$. Simulation run with 10,000 measurements every 50 sweeps, 1,000 sweep thermalization. \chris{Please make axis labels bigger.}}
\end{figure}
...
\begin{figure}[h!]
    \begin{center}
      ...
      \begin{subfigure}[b]{0.45\textwidth}
          ...
          \caption{$\tau = 5t_0$}
      \end{subfigure}
      \caption{\label{fig:divergence} $\chi_t\xi_2^2$ as a function of $L$. We fit the data with both a logarithmic and power fit. Simulation run with 10,000 measurements, once every 50 sweeps, 1,000 sweep thermalization. In the $\tau=0$ case, we have compared our result with the curve fit found in \cite{bietenholz2018}. \chris{Please make axis labels bigger.}}
    \end{center}
\end{figure}
...
% Further discussions\footnote{We would like to thank Daniel Nogradi for these conference discussions} \cite{nogradi2012}
