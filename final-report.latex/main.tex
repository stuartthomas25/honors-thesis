...
Unlike the $phi^4$ model, which consists of a real value at each point in spacetime, the $O(3)$ NLSM consists of a 3D unit vector at each point. For this reason, every transformation of the field must be ``norm preserving''. Per its name, the non-linear sigma model features a global symmetry under the 3D orthogonal group $O(3)$. In other words, the theory is invariant under rotating all vectors the same way. To differentiate it from the $\phi^4$ model, we denote the NLSM field as $\e(x)$.
...
In order to make the non-linear sigma model topologically nontrivial, we introduce a $\theta$ term into the action such that 
\begin{equation}
    S[\e] \rightarrow S[\e] - i \theta Q[\phi],
\end{equation}
where $Q$ is the topological charge (see Sec.~\ref{sec:topological charge}).
...
\begin{table}[h]
    ...
    \begin{tabular}{ c|| c | c | c}
        ...
        $\chi_m$ & 1274.05& 22.37 & 1.98\\  
        $\S_E/L^2$ & 0.29 & 0.40 & 0.44
    \end{tabular}
    \end{center}
    \caption{\label{tab:primary observables} Primary observables with average action per site corresponding to the particular configurations in Fig.~\ref{fig:primary observables}.}
\end{table}
...
\end{figure}
In the NLSM, the field at each of these vertices is a point on the sphere $S^2$. Therefore, there is a signed area $A$ on the sphere associated with each triplet of points (the sign changes with odd permutations of the ordering). We follow the derivation in \cite{berg1981} and split the plaquette into two triangles, as shown in Fig.~\ref{fig:plaquette}, with the ordering determining the sign. The topological charge density is defined using this signed area as
\begin{equation}
...
%TODO: make this in black and white
\begin{figure}[h]
  \centering
...
\begin{equation}
    ...
    S_{\mathrm{lat}}[\phi] = \sum_i \left[-\phi(x_i + a\hat{t}) - \phi(x_i + a\hat{x}) + \left(2+\half m_0^2\right)\phi^2(x_i) + \frac{1}{4}\lambda \phi^4(x_i)\right]
\end{equation}
...
We define a cluster $C$ as a continuous set of aligned sites which is generated probabilistically. In $\phi^4$ theory, we define ``aligned'' as having the same sign. In the non-linear sigma model, we choose a random unit vector $\vec{r}$ and use it to project the field vector. The sign of this quantity defines ``aligned'' in the $O(3)$ NLSM. In order to find the probability $p_i$ of adding a site $x_i$ to the cluster $C$, we use the detailed balance equation (Eq.~\ref{eq:detailedbalance}) which relates the Markov chain probability $P(\phi_a \rightarrow \phi_b)$ to the actions of each configuration. In the context of the Wolff cluster algorithm, we will notate this probability as $P\left(\phi \rightarrow f_C\left(\phi\right)\right)$ and express it as 
\begin{equation}
    P\left(\phi \rightarrow f_C\left(\phi\right)\right) = \left( \prod_{i \in C} p_i \right) \left( \prod_{j \in N} \left(1 - p_j \right)\right)
\end{equation}
where $N$ is the set of aligned neighboring sites. Since $p_i=0$ for sites that are not aligned with the cluster, they are not included in this expression.  Now if we consider the probability of flipping the cluster back, i.e. $P\left(f_C\left(\phi\right) \rightarrow \phi\right)$, we achieve a similar expression:
\begin{equation}
    P\left(f_C\left(\phi\right) \rightarrow \phi  \right) = \left( \prod_{i \in C} p_i \right) \left( \prod_{j \in M} \left(1 - p_j \right)\right)
\end{equation}
...
\begin{equation}
    \frac{ \prod_{j \in N} \left(1 - p_j \right) }{ \prod_{j \in M} \left(1 - p_j \right) } = e^{-\left(S[f_c(\phi)] - S[\phi]\right)}.
\end{equation}

\textit{TODO: add end of derivation}

\begin{equation}
    p_i = 1-e^{- \beta \vec e_a \cdot \vec e_b}.
\end{equation}
 % TODO: derive this
...
\begin{figure}[h]
  \centering
      ...
      \end{subfigure}
      \hfill
      \caption{\label{fig:wolff} An example of the Wolff cluster algorithm in the $\phi^4$ model. White represents negative values of $\phi$ while black represents positive. $\lambda=0.5$, $m_0^2=-0.9$}
