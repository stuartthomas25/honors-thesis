...
We define a cluster $C$ as a continuous set of aligned sites which is generated probabilistically. In $\phi^4$ theory, we define ``aligned'' as having the same sign. In the non-linear sigma model, we choose a random unit vector $\vec{r}$ and use it to project the field vector. The sign of this quantity defines ``aligned'' in the $O(3)$ NLSM. In order to find the probability $p_i$ of adding a site $x_i$ to the cluster $C$, we use the detailed balance equation (Eq.~\ref{eq:detailedbalance2}) which relates the Markov chain probability $P(\phi_a \rightarrow \phi_b)$ to the actions of each configuration. In the context of the Wolff cluster algorithm, we will notate this probability as $P\left(\phi \rightarrow f_C\left(\phi\right)\right)$ and express it as 
\begin{equation}
...
\begin{equation}
    \frac{ \prod_{j \in N} \left(1 - p_j \right) }{ \prod_{j \in M} \left(1 - p_j \right) } = e^{S[\phi] - S[f_C(\phi)]}.
\end{equation}
To simplify the exponent, we follow the discretized $\phi^4$ action in Eq.~\ref{eq:phi4 discretized action}. We see that the mass and interaction terms are invariant under a sign change of $\phi$ and therefore disappear in the difference. In fact, the only change in the action results from the neighboring sites $N\cup M$. The first two terms of Eq.~\ref{eq:phi4 discretized action} can be viewed as a sum over all pairs of neighboring lattice points $x_a$ and $x_b$, such that a change 
\begin{align}
    \phi(x_a) &\rightarrow -\phi(x_a) \\
    \phi(x_b) &\rightarrow \phi(x_b)
\end{align}
implies
\begin{equation*}
